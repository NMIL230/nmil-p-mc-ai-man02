#!/usr/bin/env python3
"""Supplementary Fig. S-02: Adaptive posterior surfaces with observed trials."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate supplementary adaptive posterior surfaces with observed trials",
    )
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("manuscript_figures") / "FigS01",
    ...
    parser.add_argument("--iqr-multiplier", type=float, default=CORR_IQR_MULTIPLIER_DEFAULT, help="IQR multiplier")
    parser.add_argument("--z-thresh", type=float, default=CORR_ZSCORE_THRESHOLD_DEFAULT, help="Z-score threshold")
    parser.add_argument(
        "--sort-mode",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Annotate each subplot with the chosen sort metric (default ON)",
    )
    parser.add_argument(
        "--no-show-sort-metric",
        dest="show_sort_metric",
        action="store_false",
        help="Disable subplot sort metric annotations",
    )
    parser.set_defaults(show_sort_metric=True)
    parser.add_argument(
        "--no-use-gp-surface",
        dest="use_gp_surface",
        action="store_false",
        help="Render Norm-CDF surface instead of GP posterior",
    )
    parser.add_argument(
        "--use-gp-surface",
        dest="use_gp_surface",
        action="store_true",
        help="Render GP posterior surface (default)",
    )
    parser.set_defaults(use_gp_surface=True)
    ...
    parser.add_argument(
        "--show-classic-threshold",
        dest="show_classic_threshold",
        action="store_true",
        help="Overlay classic $\psi_{\theta}$ (default ON)",
    )
    parser.add_argument(
        "--no-show-classic-threshold",
        dest="show_classic_threshold",
        action="store_false",
        help="Disable classic $\psi_{\theta}$ marker",
    )
    parser.set_defaults(show_classic_threshold=True)
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    fits_x, fits_y, ent_pairs, _pairs, valid_ent_pairs, _labels, gp_by_pid, adaptive_points_all = build_fits_and_pairs(args.pickle)
    ...
    if args.apply_validity_mask:
        ...
        valid_pids = set(gp_by_pid.keys()) | set(adaptive_points_all.keys())
    ...
    if args.exclude_outliers and len(pairs_for_outliers) >= 3:
        ...
        print("Order: completed optional outlier exclusion; proceeding to surface plotting on filtered set.")
    else:
        print("Order: no outlier exclusion requested; proceeding to surface plotting.")
    ...
    adaptive_points_all = {
        pid: adaptive_points_all[pid]
        for pid in adaptive_points_all
        if pid in valid_pids
    }

    diagnostics: Dict[str, Any] = {}
    generators = build_norm_cdf_generators(gp_by_pid, fits_x, diagnostics=diagnostics)
    ...
    plot_adaptive_probability_surface(
        generators,
        output_dir=args.out,
        filename_stem=f"FigS01_surface_sort-{args.sort_mode}",
        ...
        points_by_entity=adaptive_points_all,
        classic_thresholds=classic_scores,
        show_classic_threshold=args.show_classic_threshold,
    )
    print(f"Wrote supplementary adaptive surface figure to {args.out}")
